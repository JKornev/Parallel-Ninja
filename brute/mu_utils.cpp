...
/*
void build_login_packet(PMSG_IDPASS_EX* packet, const char* login, const char* passwd, 
	const char* version, const char* serial, uint32_t& counter)
{
	uint32_t len;

	memset(packet, 0, sizeof(PMSG_IDPASS_EX));

	packet->h.c = 0xC1;
	packet->h.size = counter++;
	packet->h.headcode = 0xF1;

	packet->subcode = 1;

	len = strlen(login);
	memcpy(packet->Id, login, len <= sizeof(packet->Id) ? len : sizeof(packet->Id));
	len = strlen(passwd);
	...
	auth_encrypt(packet, 3, 4, 1);//ok
	auth_encrypt(packet, 4, 14, 1);
	auth_encrypt(packet, 14, 34, 1);
	auth_encrypt(packet, 34, 38, 1);

	for (int i = 0; i < sizeof(packet->CliVersion); i++)
		auth_encrypt(packet, 38 + i, 39 + i, 1);
	for (int i = 0; i < 16; i++)
		auth_encrypt(packet, 43 + i, 44 + i, 1);

	auth_encrypt(packet, 59, 60, 1);
